...
in=(in+1)%8;
counter++;
printf("[Producer %d] produced item %d -> shelf has %d item(s)\n",id,item,counter);
...
out=(out+1)%8;
counter--;
printf("[Consumer %d] consumed item %d -> shelf has %d item(s)\n",id,item,counter);
...
if(pthread_create(&producer2,NULL,producer,(void *)&id2)!=0){
  perror("pthread_create");
}
